...
const Culture = () => {
    ...
    // console.log("From Culture :", fetchingSuccessful)

    // Filter posts based on the category (e.g., 'lifestyle')
    const CulturePosts = fetchingSuccessful.filter((post) =>  post.Category === 'Culture');

    console.log("Culture Posts:", CulturePosts);
    ...
    return (
        <div className="bg-[#142030] w-full h-full px-4 md:px-16 py-8">
            <div className="bg-[#5469d4] w-full p-8  ">
                <h1 className="text-2xl md:text-4xl text-slate-200 text-center">CULTURE</h1>
            </div>
            <div className=" grid md:grid-cols-3 gap-6 mt-8  ">
                <div className="col-span-2 bg-opacity-20 bg-gray-500 p-4 h-fit">
                    <div className="w-full my-5">
                        {CulturePosts && CulturePosts.map((post) => (
                            <div key={post._id} className="latest-post grid md:grid-cols-2 gap-4">
                                <div className="my-3">
                                ...
                                <Link to={`/posts/${post._id}`} className="mt-6">
                                    <PostDetails title={post.Title} author={post.Author} date={dateNow} comments="0 Comment" postContent={post.PostContent} />
                                </Link>
                            </div>
                        ))}
                    </div>
