...
const ShowingPosts = () => {
  ...
  // Get all comments
  useEffect(() => {
    fetch(`https://ultimate-app.onrender.com/Api/Comments/allComments/${id}`)
      .then(res => res.json())
      .then(data => {
  ...
  // Posting endpoint
  const postEndpoint = `https://ultimate-app.onrender.com/Api/Comments/postComment`;
  ...
  const handleReply = async () => {
    ...
    try {
      const response = await axios.post("https://ultimate-app.onrender.com/Api/Comments/postReply", ReplyToCommentData)
      console.log(response.data)
